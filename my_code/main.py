...
def validation(DTR, LTR):
    ...
    #L = [1e-4]  #lambda regularization term
    ...
    #L = [1e-2]  #lambda regularization term
    #validation_quad_LR(DTR, LTR, L, 'RAW_', PCA_Flag=True, gauss_Flag=False, zscore_Flag=False)        #RAW features
    ...
    #validation_SVM(DTR, LTR, K_arr, C_arr, 'RAW_', gauss_Flag=False, zscore_Flag=False)                  #RAW features 
    #validation_SVM(DTR, LTR, K_arr, C_arr, 'GAUSSIANIZED_', gauss_Flag=True, zscore_Flag=False)          #Gaussianized features
    #validation_SVM(DTR, LTR, K_arr, C_arr, 'ZNORMALIZED_', gauss_Flag=False, zscore_Flag=True)           #Z-normed features
    #K_arr = [1.0]
    #C_arr = [1.0]
    #SVM_score_calibration(DTR, LTR, K_arr, C_arr, appendToTitle="SVM_SCORE_CALIBRATION_", PCA_Flag=True, gauss_Flag=False, zscore_Flag=False)


    #print("############    Support Vector Machine - Quadratic (polynomial kernel)    ##############")
    #K_arr = [1., 10.]
    #validation_SVM_polynomial(DTR, LTR, K_arr, 1.0, 'RAW_', [1000], PCA_Flag=False, gauss_Flag=False, zscore_Flag=False)

    #K_arr = [0.1, 1., 10.]
    # C_arr = [1., 10.]
    

    #print("############    Support Vector Machine - Radial Basis Function kernel    ##############")
    K_arr = [1.0]
    
    #only raw data no PCA
    ...
    #validation_GMM_tot(DTR, LTR, 0.5) #for estimating 
...
def count_samples():
    ...
    for i in LTE:
        if(i==0):
            ...
            female_DTE+=1
    print(male_DTR,female_DTR,male_DTE,female_DTE)
...
if __name__ == "__main__":
    ...
    '''Notice that our training set and test set will have vertical samples!
                    +-----------+---------+---------+---------+ 
    ...
    #plot_features(DTR, LTR, appendToTitle='Z_NORMED_') 
     
    #DTR_GAUSS = gaussianize_features(DTR, DTR)
    #plot_features(DTR_GAUSS, LTR, appendToTitle='GAUSSIANIZED_')
    
    #!print("############    Validation    ##############")
    validation(DTR, LTR)    #training set 
    
    #!print("############    Evaluation    ##############")
    #evaluation(DTR, LTR, DTE, LTE) #test set 

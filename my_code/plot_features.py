...
def plot_features_histograms(DTR, LTR, _title):
    ...
    for i in range(12): #12 histograms: 1 for each feature 
        labels = ["male", "female"]
        title = _title + str(i)
        plt.figure()
        plt.title(title)

        y = DTR[:, LTR == 0][i] #DTR[:, LTR==0] take every row from DTR matrix where the corresponding
                                #index element in LTR array has value 0 (male). Then, for each row (feature) [i] plot hystogram
...
#####################################################################
'''Pearson's coefficient for HEAT-MAPS'''
...
def plot_correlations(DTR, title, cmap="Greys"):
    ...
    for x in range(12):
        for y in range(12):
            X = DTR[x, :] #select whole features x in respect to feature y
            Y = DTR[y, :]
...
def plot_features(DTR, LTR, m=2, appendToTitle=''):
    ...
    #generic heat-map on ALL training set 
    plot_correlations(DTR, "heatmap_" + appendToTitle)
    
    #heat-maps internal to the 2 classes

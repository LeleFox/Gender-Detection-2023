...
def bayes_error_min_act_plot_GMM(D, LTE, pi, title, ylim):
    p = numpy.linspace(-4, 4, 30)
    pylab.title(title)
    pylab.plot(p, bayes_error_plot(p, D, LTE, minCost=False), color='b', label='actDCF')
    pylab.plot(p, bayes_error_plot(p, D, LTE, minCost=True), 'b--', label='minDCF')
...
def validation_GMM_ncomp(DTR, LTR, pi, n, zscore=False, gauss=False):
    ...
    bayes_plot_bestGMM("raw_", 1.1, pi, GMM_llrs_raw, GMM_llrsn_raw, GMM_llrst_raw, GMM_llrsnt_raw, GMM_labels_raw)
    ...
    bayes_plot_bestGMM("gauss_", 1.1, pi, GMM_llrs_g, GMM_llrsn_g, GMM_llrst_g, GMM_llrsnt_g, GMM_labels_g)

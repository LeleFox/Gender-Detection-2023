...
def kfold_SVM(DTR, LTR, K, C, appendToTitle, PCA_Flag=False, gauss_Flag=False, zscore_Flag=False):
    k = 5
    ...
    #bayes_error_min_act_plot(scores_append, SVM_labels, appendToTitle + 'SVM, K=' + str(K) + ', C=' + str(C), 1.1)
...
def validation_SVM(DTR, LTR, K_arr, C_arr, appendToTitle, PCA_Flag=False, gauss_Flag=False, zscore_Flag=False):
    for K in K_arr:
        for C in C_arr:
            kfold_SVM(DTR, LTR, K, C, appendToTitle, PCA_Flag, gauss_Flag, zscore_Flag)

    
    #tuning curves for C parameter

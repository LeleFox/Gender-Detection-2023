...
def kfold_SVM_RBF(DTR, LTR, appendToTitle, C=1.0, K=1, gamma=1, PCA_Flag=False, gauss_Flag=False, zscore_Flag=False):
    ...
    bayes_error_min_act_plot(scores_append, SVM_labels, appendToTitle + 'SVM_RFB, K=' + str(K) + ', C=' + str(C), 1.1)
...
def validation_SVM_RBF(DTR, LTR, K_arr, gamma_arr, appendToTitle, PCA_Flag=False, gauss_Flag=False, zscore_Flag=False):
    for K in K_arr:
        for gamma in gamma_arr:
            kfold_SVM_RBF(DTR, LTR, appendToTitle, C=10.0, K=K, gamma=gamma, PCA_Flag=False, gauss_Flag=False, zscore_Flag=False)

    x = numpy.logspace(-5, 5, 21)  
    count = 1
    y = numpy.array([])

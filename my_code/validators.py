...
#!compute_min_DCF() receives: 
#!   scores: log-likelihood ratios for each sample
#!   labels: real class lable
#!   pi: prior true class
#!   Cfn, Cfp: Cost false negative and Cost false positive

#!for min_DCF we try to find the OPTIMAL treshold t in a range [-infinte, s_1,...,s_m, +infinite] where s_1,s_m are the test scores! 
#!For each threshold t, compute the confusion matrix on the test set itself that would be obtained if scores were thresholded at t, and the corresponding normalized DCF using the code developed in the previous section. The minimum DCF is the minimum
